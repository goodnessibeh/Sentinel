...
name: "Phishing Email Delivered with Malicious Attachment"
description: |
  This detection identifies inbound emails that were successfully delivered to a user's mailbox despite being flagged as phishing or containing malware. Email remains the primary initial access vector for most threat actors, and a delivered malicious attachment means the user could execute it at any time.
  A SOC analyst should act on this immediately because a malicious email has already landed in a user's inbox and the user may open the attachment at any moment.
severity: Medium
status: Available
