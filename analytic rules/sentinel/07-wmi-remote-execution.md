...
```yaml
id: 07a8b9c0-d1e2-4f3a-4b5c-6d7e8f9001b3
name: "WMI Remote Execution"
description: |
  This detection identifies processes spawned by the WMI Provider Host (wmiprvse.exe), which indicates remote code execution via Windows Management Instrumentation. WMI is a powerful administration framework that attackers frequently abuse for lateral movement and remote execution because it leaves minimal forensic artifacts.
  A SOC analyst should investigate because WMI-based remote execution is a hallmark of advanced adversaries and is commonly used in hands-on-keyboard lateral movement.
severity: Medium
status: Available
